...
syncHomeFiles_rsync() \
{
	...
	do
		rsync -ahPv --exclude=".*" "$dotfile" "$HOME/.$(basename "$dotfile")"
...
syncShellFilesFolders_sync() \
{
	...
	rsync -ahPv --exclude=".*" "$DOTFILES/home/config/zsh/" "$ZDOTDIR"
	...
	rsync -ahPv --exclude=".*" "$DOTFILES/home/config/bash/themes" "$BASHDOTDIR"

	while IFS= read -r -d '' brc;
	do
		rsync -ahPv --exclude=".*" "$brc" "$HOME/.$(basename "$brc")"
		printf "%b" "Changing permissions: " && chmod -v 700 "$HOME/.$(basename "$brc")"
	done \
	< <(find "$DOTFILES/home/config/bash" -name "bash*" -type f -print0)


	rsync -ahPv --exclude=".*" "$DOTFILES/home/config/shell/" "$SHELL_HOME"
...
syncConfigFolders_rsync() \
{
	...
	in
		"Darwin")
			rsync -ahPv --exclude=".*" --exclude="bash" --exclude="iterm2" \
				--exclude="spotify" --exclude="sublime-text" --exclude="sublime-merge" \
				--exclude="vscode" --exclude="zsh" --exclude="i3" --exclude="polybar" \
				--exclude="terminator" --exclude="X11" \
				"$DOTFILES"/home/config/ "$HOME"/.etc

			rsync -ahPv --exclude=".*" \
			"$DOTFILES"/home/config/iterm2/com.googlecode.iterm2.plist \
			"$HOME"/.config/iterm2/com.googlecode.iterm2.plist
		;;
		"Linux")
			rsync -ahPv --exclude=".*" --exclude="bash" --exclude="iterm2" \
				--exclude="spotify" --exclude="sublime-text" --exclude="sublime-merge" \
				--exclude="vscode" --exclude="zsh" --exclude="deluge" --exclude="karabiner" \
				"$DOTFILES"/home/config/ "$HOME"/.etc
		;;
	esac
	echo
...
syncLocalFolders_rsync() \
{
	...
	sudo rsync --exclude=".*" -ahPv "$DOTFILES/home/root/opt/oh-my-zsh/" "/opt/oh-my-zsh"
	echo
}
...
OPTIONS:
    -d,--dotfiles       Place the Dotfiles in $HOME/$XDG with:
       home                 Rsync corresponding files in $HOME
       ...
       shell                Rsync corresponding Shell files in $XDG_CONFIG_HOME
       config               Rsync custom files in $XDG_CONFIG_HOME
       local                Rsync custom files in $XDG_DATA_HOME
	...
	case $1 in
		...
		"-d"|"--dotfiles")
			case $2 in
				...
				;;
				"config")
					echo; syncConfigFolders_rsync
				;;
				"local")
					echo; syncLocalFolders_rsync
				;;
				"etcfolders")
					echo; syncEtcFolders
				;;
