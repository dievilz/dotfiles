...
subopt "Enable volume key feedback."
...
saferun sudo defaults write /Library/Preferences/.GlobalPreferences MultipleSessionEnabled -bool true



subopt "Enable firewall - Way #1."
saferun sudo defaults write /Library/Preferences/com.apple.alf globalstate -int 1

subopt "Disable guest account access to shared folders."
saferun sudo defaults write /Library/Preferences/com.apple.AppleFileServer guestAccess -bool true
...
then
	# s-aferun s-udo defaults write /Library/Preferences/com.apple.loginwindow AdminHostInfo -string "HostName"
	saferun sudo defaults delete /Library/Preferences/com.apple.loginwindow AdminHostInfo
fi

subopt "Enable guest account login."
saferun sudo defaults write /Library/Preferences/com.apple.loginwindow GuestEnabled -bool true

subopt "Display login window as: Name and password."
saferun sudo defaults write /Library/Preferences/com.apple.loginwindow SHOWFULLNAME -bool true

subopt "Show language menu in the top right corner of the login screen."
saferun sudo defaults write /Library/Preferences/com.apple.loginwindow showInputMenu -bool true




subopt "Enable HiDPI display modes (requires restart)."
saferun sudo defaults write /Library/Preferences/com.apple.windowserver DisplayResolutionEnabled -bool true




subopt "Disable \"Restart automatically if the computer freezes\" - Won't work."
saferun sudo systemsetup -setrestartfreeze Off

subopt "Enable Good Ol' Startup Chime."
saferun sudo nvram StartupMute=%00   ## %01 to disable

subopt "Enforce System Hibernation."
saferun sudo pmset -a hibernatemode 25

subopt "Disable Power Nap."
saferun sudo pmset -a powernap 0
# saferun sudo pmset -a standby 0
# saferun sudo pmset -a standbydelay 0
# saferun sudo pmset -a autopoweroff 0
...
saferun sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.locate.plist
...
subopt "Disable automatically rearrange Spaces based on recent use."
...
subopt "Disable auto-switching desktops."
...
suboptq "Choose one option [${BO}'Y'es${NBD}|${BO}'n'o${NBD}][press ${BO}Enter " \
"key${NBD} to halt]: "
read -r resp
case "$resp" in
	y|Y|yes|Yes)
...
# saferun s-udo dscl . -delete /Groups/admin GroupMembership "${demotedAdminName}"
# saferun s-udo dscl . -delete /Groups/admin GroupMembers "$(dscl . -read /Users/"${demotedAdminName}" GeneratedUID | awk '{print $2}')"

# saferun dsscl . -change /Users/<myShortName> RealName <myShortName> <myNewShortName>
# saferun s-udo dscl . -list "/SharePoints"
...
case "$resp" in
	y|Y|yes|Yes)
		echo
		info "Remember that the Computer Name is the one that will appear" \
		"in your iCloud's Devices List and in the \"Find My\" app."
		echo
...
case "$resp" in
	y|Y|yes|Yes)
		# echo
		# info "Remember that the Hostname is the one that will appear in" \
		# "."
		echo
...
case "$resp" in
	y|Y|yes|Yes)
		# echo
		# info "Remember that the Local Hostname is the one that will appear" \
		# "in ."
		echo
...
case "$resp" in
	y|Y|yes|Yes)
		# echo
		# info "Remember that the Real Name is the one that will appear" \
		# "in ."
		echo
...
suboptq "Choose one option [${BO}'u'rl${NBD}|${BO}'g'ravatar${NBD}|${BO}'l'ocal" \
"${NBD}][press ${BO}Enter key${NBD} to halt]: "
read -r resp
case "$resp" in
	u|U)
		echo
...
saferun sudo /usr/libexec/ApplicationFirewall/socketfilterfw --setglobalstate on
## Firewall is enabled. (State = 1)

trdopt "Enable Firewall's Logging Mode."
saferun sudo /usr/libexec/ApplicationFirewall/socketfilterfw --setloggingmode on
## Turning on log mode

trdopt "Enable Firewall's Stealth Mode."
saferun sudo /usr/libexec/ApplicationFirewall/socketfilterfw --setstealthmode on
...
saferun sudo /usr/libexec/ApplicationFirewall/socketfilterfw --setallowsigned off

trdopt "Disable allow signed downloaded applications automatically."
saferun sudo /usr/libexec/ApplicationFirewall/socketfilterfw --setallowsignedapp off
...
saferun sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.captive.control.plist Active -bool false
echo
...
then
	...
	saferun sudo chflags schg "$HOME"/Library/Preferences/com.apple.LaunchServices.QuarantineEventsV2
	echo
fi
...
saferun sudo ifconfig en0 ether $(openssl rand -hex 6 | sed 's%\(..\)%\1:%g; s%.$%%')
echo
...
saferun sudo defaults write /Library/Preferences/com.apple.mDNSResponder.plist NoMulticastAdvertisements -bool true
...
OPTIONS:
    ...
    -p,--post-install   Set up post-install system configurations (including @DrDuh Privacy Guide)
